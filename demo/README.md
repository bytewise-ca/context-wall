# ContextWall context firewall - Demo

Side-by-side demonstration of a web-browsing Claude agent with and without the ContextWall context firewall.
...
| `WITH_CONTEXTWALL` | Behaviour |
|---|---|
| `true` (default) | Web results pass through ContextWall before Claude sees them. Injection attempts are blocked. |
| `false` | Web results go directly to Claude. The agent is unprotected. |

Switch modes by editing `WITH_CONTEXTWALL` in `.env` and restarting.
...
**With ContextWall:** The instruction-override pattern is detected by the built-in injection detector (multi-layer: structural + regex + heuristic). The document is blocked before Claude sees it.

**Without ContextWall:** Claude processes the injected instruction and attempts to comply.
...
**With ContextWall:** The `disregard` instruction pattern triggers the built-in injection detector. Blocked.

**Without ContextWall:** Claude's research goal is silently redirected.
...
Demo Agent (port 8081)
   ...
   ├── ContextWall context firewall (port 8080)  ← inline filter
