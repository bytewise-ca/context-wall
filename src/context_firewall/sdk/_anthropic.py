...
Environment variables (no code changes needed)::
    ...
    CONTEXTWALL_KEY=sk-cre-...
    CONTEXTWALL_URL=http://localhost:8080

    from contextwall_sdk import SafeAnthropic
    client = SafeAnthropic()  # reads from env

The legacy CRE_KEY / CRE_URL names are still accepted (with a
DeprecationWarning) so existing setups keep working. Support for the
legacy names will be removed in v0.3.
"""
...
from ._compat import getenv_with_legacy, is_policy_violation_type
...
def _check_cre_block(body: Any) -> None:
    """Raise ContextWallBlockedError if the response body is a ContextWall policy violation."""
    if not isinstance(body, dict):
        return
    err = body.get("error", {})
    if isinstance(err, dict) and is_policy_violation_type(err.get("type")):
...
class SafeAnthropic:
    ...
    Args:
        cre_key:               Your ``sk-cre-xxx`` key. Falls back to
                               ``CONTEXTWALL_KEY`` (or legacy ``CRE_KEY``) then
                               ``ANTHROPIC_API_KEY`` env vars.
        cre_url:               ContextWall daemon URL. Falls back to
                               ``CONTEXTWALL_URL`` (or legacy ``CRE_URL``),
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._cre_url = (
            cre_url or getenv_with_legacy("CONTEXTWALL_URL") or "http://localhost:8080"
        ).rstrip("/")
        ...
        key = (
            cre_key
            or getenv_with_legacy("CONTEXTWALL_KEY")
