...
and querying analytics.

Example::

    from contextwall_sdk import ContextWallClient

    cw = ContextWallClient(api_key="...", base_url="http://localhost:8080")

    # Provision a key for an agent
    result = cw.keys.create(
        project_id="my-agent",
...
from ._compat import getenv_with_legacy
...
def _resolve_key_and_url(api_key: str | None, base_url: str | None) -> tuple[str, str]:
    """Env-var resolution shared by both client variants.

    Reads CONTEXTWALL_* first; falls back to the deprecated CRE_* names with
    a DeprecationWarning. Support for the legacy names ends in v0.3.
    """
    key = (
        api_key
        or getenv_with_legacy("CONTEXTWALL_API_KEY")
        or getenv_with_legacy("CONTEXTWALL_API_TOKEN")
        or ""
    )
    url = (
        base_url
        or getenv_with_legacy("CONTEXTWALL_URL")
...
class ContextWallClient:
    ...
    Args:
        api_key:  ContextWall admin API key. Falls back to
                  ``CONTEXTWALL_API_KEY`` then ``CONTEXTWALL_API_TOKEN``
                  env vars (legacy ``CRE_*`` names accepted with a
                  DeprecationWarning).
        base_url: ContextWall daemon URL. Falls back to ``CONTEXTWALL_URL``,
    ...
    def __init__(
        ...
        key, url = _resolve_key_and_url(api_key, base_url)
